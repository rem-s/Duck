...
class MultiChannelMicWindow(BaseWindow):
    ...
    def __init__(self, recorder):
        ...
        self.radius = 60 # y-axis max
        self.r_width = self.radius // 10
    ...
    def update(self):
        ...
        theta = random.randint(0, all_direction//self.nbins)

        self.direction[theta] = min(self.direction[theta]+1, self.radius)
        
        if self.n_update % 64 == 0:
            self.n_update = 0
            self._widget.clear()
            # Add polar grid lines
            # self._widget.addLine(x=0, pen=0.2)
            # self._widget.addLine(y=0, pen=0.2)
        ...
        self._widget.plot(x=[x], y=[y], symbol='o', symbolSize=10, symbolPen=(255,255,255,200), symbolBrush=(0,0,255,150))
